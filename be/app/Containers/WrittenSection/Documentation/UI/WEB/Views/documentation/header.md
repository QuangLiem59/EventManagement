...
The rate limit window is `1` minutes per endpoint, with most individual calls allowing for `1200` requests in each window.

*In other words, each user is allowed to make `1200` calls per endpoint every `1` minutes. (For each unique access token).*
...
The Access Token lives for `365 days, 0 hours, 0 minutes and 0 seconds`. (equivalent to `525600` minutes).
While the Refresh Token lives for `400 days, 0 hours, 0 minutes and 0 seconds`. (equivalent to `576000` minutes).

*You will need to re-authenticate the user when the token expires.*

## **Pagination**

By default, all fetch requests return the first `20` items in the list. Check the **Query Parameters** for how to control the pagination.
...
```
http://localhost/v1/endpoint?limit=100
```
...
```
http://localhost/v1/endpoint?limit=100&page=2
```
...
```
http://localhost/v1/endpoint?orderBy=created_at
```
...
```
http://localhost/v1/endpoint?orderBy=name&sortedBy=desc
```
...
```
http://localhost/v1/endpoint?search=keyword here
```
...
```
http://localhost/v1/endpoint?search=first keyword;second keyword
```

#### Search in a specific field:
```
http://localhost/v1/endpoint?search=field:keyword here
```

#### Search in specific fields for multiple keywords: 
```
http://localhost/v1/endpoint?search=field1:first field keyword;field2:second field keyword
```

#### Search in a date field:
```
http://localhost/v1/endpoint?searchDate=start date,end date|field
```
##### Usage
```
http://localhost/v1/users?searchDate=2023-01-01
http://localhost/v1/users?searchDate=,2023-01-31
http://localhost/v1/users?searchDate=2023-01-01,2023-01-31
http://localhost/v1/users?searchDate=2023-01-01,2023-01-31|birthday
```
...
```
http://localhost/v1/endpoint?searchNull=field1;field2:not
```
##### Usage
```
http://localhost/v1/users?searchNull=birthday
http://localhost/v1/users?searchNull=birthday:not
http://localhost/v1/users?searchNull=birthday;gender:not
```
The above example will execute the following query:
...
#### Search with include
```
http://localhost/v1/endpoint?searchInclude=include:field,value
```
#### Usage
```
http://localhost/v1/orders?searchInclude=user:name,admin
```

#### Define query condition:

```
http://localhost/v1/endpoint?search=field:keyword&searchFields=name:like
```
...
```
http://localhost/v1/endpoint?search=field1:first keyword;field2:second keyword&searchFields=field1:like;field2:=;
```
...
```
http://localhost/v1/endpoint?search=age:17;email:john@gmail.com
```
...
```
http://localhost/v1/endpoint?search=age:17;email:john@gmail.com&searchJoin=and
```
...
```
http://localhost/v1/endpoint?filter=id;status
```
...
```
http://localhost/v1/endpoint?page=200
```
...
```json
  ...
  "meta": {
    "pagination": {
      ...
      "total_pages": 1111,
      "links": {
        "previous": "http://http://localhost/v1/endpoint?page=21"
      }
    }
  }
```
...
```
http://localhost/v1/endpoint?include=relationship
```
...
```
http://localhost/v1/endpoint?skipCache=true
```
...
```shell
curl -X POST -H "Accept: application/json" -H "Content-Type: application/x-www-form-urlencoded; -F "email=admin@admin.com" -F "password=admin" -F "=" "http://http://localhost/v2/register"
```

Calling protected endpoint (passing Bearer Token) example:

```shell
curl -X GET -H "Accept: application/json" -H "Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..." -H "http://http://localhost/v1/users"
```
