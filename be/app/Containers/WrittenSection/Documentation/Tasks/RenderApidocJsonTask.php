...
class RenderApidocJsonTask extends Task
{
    ...
    {
        $this->templatePath = 'Containers/' . config('writtenSection-documentation.section_name') . '/Documentation/ApiDocJs/' . $docType . '/apidoc.json';
        $this->outputPath = 'Containers/' . config('writtenSection-documentation.section_name') . '/Documentation/ApiDocJs/' . $docType . '/apidoc.json';
        $this->replaceArray = [
            'name' => config('app.name'),
            'description' => config('app.name') . ' (' . ucfirst($docType) . ' API) Documentation',
            'title' => 'Welcome to ' . config('app.name'),
            'url' => $this->getFullUrl(),
            'sampleUrl' => config('writtenSection-documentation.enable-sending-sample-request') ? $this->getFullUrl() : null,
        ];
    }
